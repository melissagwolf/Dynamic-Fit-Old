...
```{r}
#Did they enter unstandardized loadings?
unstandardized <- function(model){
  Mod <- read.delim(model()$datapath, header = FALSE)   
  Mod_C <- as.character(Mod$V1)             
  
  lav_file <- lavaanify(Mod_C, fixed.x=FALSE) %>%  
...
```{r}
#If user tries to enter a model statement with a loading or correlation of 1+
#Creating a new action button here (input$pro) to call later
FirstWarning <- function(){
    modalDialog(
      title="Warning",
      "It looks like you may have entered unstandardized loadings or covariances (loading or covariances that are equal to 1). This program requires standardized loadings and correlations. Any cutoffs derived from unstandardized loadings or covariances will be inaccurate.",
    footer = tagList(actionButton("pro", "Proceed anyway"),
                       modalButton("Go back")))}

#Filter people out who have loadings = 1 and show them the above warning. They can either press proceed or go back. If they go back, the modal closes.  If they proceed, it will attempt to run the model (this command is further down in the dynamicfit function)
observeEvent(input$go,{
  if(unstandardized(model())>0){
    showModal(FirstWarning())
  }else{
    return()
  }
  })
...
#If user tried to proceed with potentially unstandardized loadings
#Close modal
...
#And proceed
dynamic_fit <- eventReactive(input$pro,{
...
A comparison of the fit index distributions for the "true" empirical model and the "misspecified" empirical model.  The red line represents the dynamic model fit cutoff value for the user's empirical model.  The black line represents the commonly used cutoff values recommended by Hu & Bentler (1999).
...
renderPlot({
  ...
  SRMR_plot <- plot %>% 
    ...
    scale_color_manual(name="Cutoff Values",
                       labels=c("Hu & Benter Cutoff","Misspecified Cutoff"),
