...
library(simstandard)
library(tools)
library(patchwork)  ##add this to server?
library(tibble)
library(magrittr)
library(purrr)    ##add this to server?
library(stringr)  ##add this to server?
...
Misspecified_DGM_Multi <- function(model){
  ...
  multi_mod <- lapply(seq(nrow(mod2)), function(x) rbind(model,mod2[seq(x), ,drop = FALSE]) %>%
                        data.frame() %>% 
                        pull(V1))
