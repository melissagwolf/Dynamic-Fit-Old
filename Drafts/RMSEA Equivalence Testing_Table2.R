...
rmsea_3 <- rmsea_results(T_ml,df,n)

good <- c("Excellent:","Close:","Fair:","Mediocre:","Poor:")

one <- paste(rmsea_3[1],"or below")
two <- paste(rmsea_3[1],"to",rmsea_3[2])
three <- paste(rmsea_3[2],"to",rmsea_3[3])
four <- paste(rmsea_3[3],"to",rmsea_3[4])
five <- paste(rmsea_3[4],"or above")
...
ggplot(data.frame(x), aes(x=x, y=0)) +
  ...
  annotate("segment",x=rmsea_3[5],xend=rmsea_3[5],y=-0.1,yend=.3, size=1, col="tomato4")+
  annotate("text",x=rmsea_3[5],y=.6,label=paste("T-size \n RMSEA \n",rmsea_3[5]),col="tomato4", size=3.5)+
...
cfi_results <- function(N,T_ml,df,p,T_mli){
...
N=487;#sample size
T_ml= 469.29; #model chi-square
df=49;#model degrees of freedom
p=20 #num parm
T_mli <- 1700 #baseline CFI


##SEHS
N=12752;#sample size
T_ml= 5412.017; #model chi-square
df=50;#model degrees of freedom
p=12 #num parm
T_mli <- 67019.332 #baseline CFI

cc <- cfi_results(N,T_ml,df,p,T_mli)
