...
Misspecified_DGM_Single <- function(model){
  ...
  single_mod <- base::lapply(levels, function(x) base::rbind(model,mod[base::seq(x), ,drop = FALSE]) %>%
                         base::data.frame() %>% 
                         dplyr::pull(V1))
...
renderTable({
  ...
  Row2 <- purrr::map_dfr(Table,~dplyr::mutate(.,SRMR_1=SRMR,
                    ...
                    dplyr::mutate(SRMR=base::ifelse(base::is.character(SRMR),SRMR_1,"--"),
                           ...
                           SRMR=base::as.character(SRMR),
                           RMSEA=base::as.character(RMSEA),
                           CFI=base::as.character(CFI)) %>% 
                    dplyr::select(SRMR,RMSEA,CFI)) 
  
  Table_C <- purrr::map_dfr(Table,~dplyr::mutate(.,SRMR=base::as.character(SRMR),
                                   RMSEA=base::as.character(RMSEA),
                                   CFI=base::as.character(CFI)))
...
renderPlot({
  ...
  Final_Plots <- base::lapply(base::seq(base::length(plots_combo)), function(x) patchwork::wrap_plots(plots_combo[[x]])+
