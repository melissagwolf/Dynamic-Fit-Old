...
#Misspecifications that are being added:
multi_factor(model)

#What the final model looks like for each level (hard to read)
Misspecified_DGM_Multi(model)

#Get fit cutoffs and graphs
multiCFA(model,n)
...
multiCFA <- function(model,n){
  ...
  if (nrow(multi_factor_num(model))<(number_factor(model)-1)){
  ...
  lapply(seq(length(plots_combo)),function(x) grid.arrange(grobs=plots_combo[[x]][1:3],ncol=3))
  
  library(patchwork)
  
  lapply(seq(length(plots_combo)), function(x) wrap_plots(plots_combo[[x]])+plot_layout(guides = "collect") & theme(legend.position = 'bottom'))
  ...
  lapply(seq(length(plots_combo)),function(x) ggarrange(grobs=plots_combo[[x]][1:3],nrow=1,ncol=3))
  
  lapply(seq(length(plots_combo)),function(x) grid_arrange_shared_legend(plots_combo[[x]],ncol=3,nrow=1))
  
  
  grid_arrange_shared_legend(test[[1]],test[[2]],test[[3]])
  
  grid_arrange_shared_legend(test[1:3])
  ...
  lapply(seq(length(plots_combo)),function(x) plot_grid(plotlist=[[x]],ncol=3))
  
  test <- plots_combo[[1]]
  
  do.call(plot_grid,test)
  
  lapply(seq(length(plots_combo)), function(x) do.call(plot_grid,[x]))
  
  do.call(plot_grid,lapply(plots_combo),ggplotGrob)
