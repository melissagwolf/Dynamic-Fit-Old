...
<html>
<head>
<title>Dynamic Fit Index Cutoffs for Multifactor CFA Models</title>
...
This app uses Monte Carlo simulations to generate dynamic fit index cutoffs for multifactor CFA models.
...
```{r misspecified_dgm}
#### Function to create Misspecified DGM for multifactor CFA ####
...
**Note**: These cutoff values are currently only accurate for single level multifactor cfa models.  Any cutoff values produced for bi-factor models or higher-order models are inaccurate (you will likely get an error message if you try to compute them).
...
renderTable({
  ...
  shiny::validate(
    ...
    "You entered a single factor model. Go back to the landing page and select the single factor model application."))  
