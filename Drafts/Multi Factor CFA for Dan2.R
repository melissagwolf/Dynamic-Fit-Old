...
#multiCFA is the function that produces the table and the plots.  It expects 4 arguments: the model statement,
#sample size, lowest level of misspecifications, and highest level of misspecifications requested by the user.
#default for lower is 1, and default for upper is lower.

multiCFA(model,n,lower,upper)

#multi_factor_num returns an ordered list of all viable items and corresponding loadings for model
#misspecifications. Only argument is the model statement.
multi_factor_num(model)

#Misspecified_DGM_Multi returns the misspecification(s) that will be added to the misspecified dgm
#doesn't include lower, because we simulate all data get the same results across seeds
#max cross-loading applied

Misspecified_DGM_Multi(model,upper)
...
#RUn code (make sure to double check which model you're running):

multi_factor_num(model)

Misspecified_DGM_Multi(model,4)

multiCFA(model,500,1,4)

#I added chi-sq and fmin to the output - it's the 25th and 75th percentile

#To see how long something takes, wrap the function within system.time()

#################### Run all of this below ###
...
multi_factor <- function(model,upper){
  
  if (nrow(multi_factor_num(model))<upper){
    stop("dynamic Error: There are no more paths to add in your model.  Select a lower level.")
  }
...
Misspecified_DGM_Multi <- function(model, upper){

  mod <- multi_factor(model,upper)
...
misspecified_model_fit <- function(model,n,lower,upper){
  ...
  misspec_dgm <- Misspecified_DGM_Multi(model,upper)
  ...
  all_levels <- map(misspec_dgm,~sim_standardized(m=.,n=n*500,
                                                        latent=FALSE,errors=FALSE))
  
  all_data_misspec <- all_levels[lower:upper]
  ...
  misspec_fit_sum <- map(misspec_cfa, function(x) map_dfr(x, function(y) fitMeasures(y, c("srmr","rmsea","cfi","fmin","chisq"))) %>% 
                           `colnames<-`(c("SRMR_M","RMSEA_M","CFI_M","fmin_M","chisq_M")) %>%
...
true_model_fit <- function(model,n){
  ...
  #Extract fit stats from each rep (list) into a data frame and clean
  true_fit_sum <- map_dfr(true_cfa,~fitMeasures(., c("srmr","rmsea","cfi","fmin","chisq"))) %>%
    `colnames<-`(c("SRMR_T","RMSEA_T","CFI_T","fmin_T","chisq_T")) %>%
...
multiCFA <- function(model,n,lower,upper){
  ...
  misspec_sum <- map(results,~summarise(.,SRMR_M=quantile(SRMR_M, c(.05,.1)),
                                        ...
                                        fmin_M=quantile(fmin_M, c(.25,.75)),
                                        chisq_M=quantile(chisq_M, c(.25,.75))))
  true_sum <- map(results,~dplyr::summarise(.,SRMR_T=quantile(SRMR_T, c(.95,.9)),
                                            ...
                                            fmin_T=quantile(fmin_T, c(.25,.75)),
                                            chisq_T=quantile(chisq_T, c(.25,.75))))
  ...
  Table <- map(misspec_sum,~cbind(.,true_sum[[1]]) %>% 
                 mutate(SRMR_R=base::round(SRMR_M,3),
                        ...
                        fmin_M=base::round(fmin_M,3),
                        fmin_T=base::round(fmin_T,3),
                        chisq_M=base::round(chisq_M,3),
                        chisq_T=base::round(chisq_T,3),
                 ...
                 dplyr::select(SRMR,RMSEA,CFI,fmin_M,fmin_T,chisq_M,chisq_T)) 
  ...
  Row2 <- map_dfr(Table,~mutate(.,SRMR_1=SRMR,
                    ...
                    mutate(SRMR=ifelse(is.character(SRMR),SRMR_1,"--"),
                           ...
                           fmin_M=as.character(fmin_M),
                           fmin_T=as.character(fmin_T),
                           chisq_M=as.character(chisq_M),
                           chisq_T=as.character(chisq_T)) %>% 
                    select(SRMR,RMSEA,CFI,fmin_M,fmin_T,chisq_M,chisq_T)) 
  ...
  Table_C <- map_dfr(Table,~mutate(.,SRMR=as.character(SRMR),
                                   ...
                                   fmin_M=as.character(fmin_M),
                                   fmin_T=as.character(fmin_T),
                                   chisq_M=as.character(fmin_M),
                                   chisq_T=as.character(chisq_T)))
  
  Table_C[seq(2,nrow(Table_C),by=2),] <- Row2 
  
  Table_C$levelnum <- paste("Level", rep(lower:upper,each=2))
